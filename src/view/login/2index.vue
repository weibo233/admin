<template>
    <div class="login-vue" :style="bg">
        <div class="container">
            ...
            <div class="input-c">
                <Input prefix="ios-contact" v-model="loginForm.name" placeholder="用户名" clearable @on-blur="verifyAccount" />
                <p class="error">{{accountError}}</p>
            </div>
            <div class="input-c">
                <Input type="password" v-model="loginForm.password" prefix="md-lock" placeholder="密码" clearable @on-blur="verifyPwd"
                @keyup.enter.native="submit" />
                <p class="error">{{pwdError}}</p>
            </div>
            <div class="input-c">
                <Input type="password" v-model="loginForm.captcha" prefix="md-lock" placeholder="验证码" clearable @on-blur="verifycaptcha"
                @keyup.enter.native="submit" />
...
export default {
    ...
    created() {
        this.bg.backgroundImage = 'url(' + require('../assets/images/bg0' + new Date().getDay() + '.jpg') + ')'
    },
    watch: {
        $route: {
            handler(route) {
    ...
    methods: {
        ...
        submit() {
            ...
            //     }
            // }
            this.$axios.post('framework/all/login')
        },
    },
}
</script>
